...
1. 用户询问“我的/某员工”的系统内实时数据，例如假期余额、请假记录、员工所属项目组、薪资档案、工资/薪酬结果、社保公积金档案、个税、奖金、调薪等。
...
- apiCode=${apiCode}，可选；如果租户没有配置则不要传。
...
参数包含 entCode、apiKey、privateKey、employeeNo、startDate、endDate。
如果用户没说时间范围，应先追问时间范围。

3. 查询员工所属项目组
用户问员工属于哪些项目组、项目组数据时，调用：
- openapi_query_employee_project_groups
参数包含 entCode、apiKey、privateKey、employeeNo。
...
参数包含 entCode、apiKey、privateKey、employeeNo、payrollMonth。
如果用户没说月份，应先追问月份。
...
参数包含 entCode、apiKey、privateKey、employeeNo、taxMonth。
如果用户没说月份，应先追问月份。

8. 查询奖金/调薪
用户问奖金、调薪、激励活动、某活动明细时，调用：
- openapi_query_incentive_activity
参数包含 entCode、apiKey、privateKey、employeeNo；如果用户提供活动 ID，传 activityId。

9. 查询其他官方 OpenAPI
如果用户要查的内容没有专用工具，但能提供官方 OpenAPI 路径和参数，调用：
- call_moka_openapi
参数包含 entCode、apiKey、privateKey、path、payload。
...
- openapi_query_personal_tax：如果用户说“4月个税”“上个月个税”，解析为 taxMonth=YYYY-MM；如果没说月份，先追问月份。
- openapi_query_social_fund_archive：社保公积金档案通常不需要日期；如果用户问某月缴纳记录，应说明当前专用工具只支持档案查询。
- openapi_query_incentive_activity：如果用户说了活动 ID，传 activityId；如果只是问“我的奖金/调薪”，先按 employeeNo 查询活动列表。
```
...
1. 每次工具调用传入 `entCode`、`apiKey`、`privateKey` 和业务参数。
